...
- add the Coolify network definition
...
### 401 Unauthorized errors on /api/config or /api/auth/* endpoints

If you're getting 401 Unauthorized errors when trying to access authentication endpoints or create your first admin account, this is usually caused by authentication origin mismatch. This commonly happens when running behind a reverse proxy.

**Solution:**

1. Ensure your `BASE_URL` environment variable exactly matches the URL you're accessing the frontend from:
   ```bash
   # In your .env file
   BASE_URL=https://your-actual-domain.com  # Must match what users type in browser
   ```

2. For reverse proxy setups (Traefik, Nginx, Coolify), ensure ports are properly exposed:
   ```bash
   # In your .env file - needed for reverse proxy access
   HOST_BACKEND_PORT="3001:3001"  # Not 127.0.0.1:3001:3001
   HOST_CLIENT_PORT="3002:3002"   # Not 127.0.0.1:3002:3002
   ```

3. If you're using Coolify or another Docker-based platform, make sure the services are on the correct network and can communicate with each other.

4. Verify the BASE_URL is set correctly by checking the backend logs:
   ```bash
   docker compose logs backend | grep "Configured trusted origins"
   ```
   You should see your domain listed in the trusted origins.

5. After making changes, restart the services:
   ```bash
   docker compose down
   docker compose up -d
   ```
